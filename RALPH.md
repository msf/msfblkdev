...
1. Read `GOAL_V0_4.md` and the relevant sections of `coding-project-tigerbeetle-railway.md` fully. Inspect `git status`, recent commits and existing code/tests before choosing work.
...
4. Before committing, run `.tools/zig-x86_64-linux-0.16.0/zig build test`. Stage explicit paths only; never use `git add .` or `git add -A`.
5. Do not modify `GOAL_V0_4.md`, `RALPH.md`, the design document or `ublksrv/`. Do not push, amend, rebase or reset existing commits.
...
Finish with the commit SHA, tests run, and the next smallest missing capability. If V0.4 acceptance already passes, make no commit and report `V0.4 COMPLETE`.
